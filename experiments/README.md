...
The synthetic traces predate `run_config` and `task_summary`. Their agent version comes from Harbor's records; cap and repetition come from the original filenames. The 0.1.0 source was not saved. The published 0.2.0 harness has only comment edits since the initial repository import. Rerunning the synthetic suite now uses 0.2.0.
